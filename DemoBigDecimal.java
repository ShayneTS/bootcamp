...
public class DemoBigDecimal {
  public static void main (String[] args) {
    ...
    double a = 0.1;
    double b = 0.2;
    result = BigDecimal.valueOf(a).multiply(BigDecimal.valueOf(b).doubleValue());
    System.out.println(result); //0.02

    result = BigDecimal.valueOf(0.125).multiply(BigDecimal.valueOf(0.35)).setScale(2, RoundingMode.HALF_UP).doubleValue();
    System.out.println(result); //0.04

    // divide (non-terminated decimal place, e.g. 10/3)
    double e = 10.0;
    double f = 3.0;
    ...
    result = BigDecimal.valueOf(0.5) //
        .divide(BigDecimal.value(4.0), 2, RoundingMode.HALF_UP) // Half 5
        .doubleValue();
