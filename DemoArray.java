public class DemoArray {
  public static void main(String[] args) {
    ...
    arr2[0] = 99L; //in fact L has no effect here, beause 99 is int , assign to Long is a promotion
    arr2[1] = 5L;
    arr2[2] = 40L;
  ...
  for (int i = 1; i< arr4.length; i++) {
    if (Math.abs(arr4[i]) > Math.abs(arr[i-1])) {
      max = Math.abs(arr4[i]);
    } else max = Math.abs(arr4[i-1]);
  }
  System.out.println("Max value in arr4 = " + max);
  // or Vincent's version, just assign initialize max = arr4[0], then replace if arr4[i > max]
  int index = 0;
  int indexMin = 0;
  int max2 = arr4[0];
  int min = arr4[0];
